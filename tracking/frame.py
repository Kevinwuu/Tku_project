...
# 讀檔
folder = "image/sample1/"
allfiles = os.listdir(folder)
file = [filename for filename in allfiles if filename[-4:] in [".jpg"]]
count = 0
line_list = []
thresh = 20
...
while(True):
    ...
    bg = Image.open(folder + file[0])
    img1 = Image.open(folder + file[count])
    img2 = Image.open(folder + file[count + 1])
    ...
    contours1 = cv2.findContours(
        dilated1, cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_SIMPLE)[1]
    contours2 = cv2.findContours(
        dilated2, cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_SIMPLE)[1]
    ...
    for i in keep_list:
        for j in line_list:
            if line_list.index(j) == i:
                distance = sum(
                    list(map(lambda x, y: (x - y)**2, j[0][0], j[-1][1])))**(1 / 2) * 30 / 500
                m1 = (j[0][1][1] - j[0][0][1]) / (j[0][1][0] - j[0][0][0])
                m2 = (j[-1][-1][1] - j[0][0][1]) / (j[-1][-1][0] - j[0][0][0])
                angle = math.degrees(math.atan((m1 - m2) / (1 + m1 * m2)))
                print("obj", line_list.index(j), "distance =", distance,
                      "speed =", distance / len(j), "angle =", angle)
